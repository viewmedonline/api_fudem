...
main() {
  ...
  if [ $? -eq 0 ]; then
    ...
    if [[ $msg == *"now locked against writes"* && $msg == *": 1"* ]]; then
      ...
      isBckDone=true
      msg=$(mongodump --uri="mongodb://$USER_AUTH:$PASSWD_AUTH@$HOST:$PORT/$DATABASE?authSource=$DATABASE_AUTH" --excludeCollection fs.files --excludeCollection fs.chunks --gzip --quiet --archive="$PATH_BACKUP/$FILE_NAME")
      if [ $? -eq 0 ]; then
