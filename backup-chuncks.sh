...
DATABASE_AUTH="admin"
PATH_BACKUP="./backups/$FOLDER"
LOGFILE="./backups/database_backup_chuncks.log"
FILE_NAME="viewmedFudemChuncks.gz"
isBckDone=false
#
################################################################################
#
...
bk_general() {
  ...
  mkdir -p $PATH_BACKUP >/dev/null 2>&1
  if [ $? -eq 0 ]; then
    ...
    if [[ $msg == *"now locked against writes"* && $msg == *": 1"* ]]; then
      ...
      isBckDone=true
      start_of_week=$(date -d '-8 days' '+%Y-%m-%dT00:00:00Z')
      log "backup from: $start_of_week"
      mongo_query='{"date": {"$gte": ISODate("'"$start_of_week"'")}}'
      msg=$(mongodump --uri="mongodb://$USER_AUTH:$PASSWD_AUTH@$HOST:$PORT/$DATABASE?authSource=$DATABASE_AUTH" --query="$mongo_query" --collection="fs.chunks" --gzip --quiet --archive="$PATH_BACKUP/$FILE_NAME")
      if [ $? -eq 0 ]; then
