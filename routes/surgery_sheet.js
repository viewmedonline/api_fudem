...
router.post("/surgery_sheet", async (request, response) => {
  try {
    ...
    const report_id = await save_file(
      `surgery_sheet_${request.body.data.num_exp}.pdf`,
      pdf_data
    );
    request.body.data.pdf = report_id;
    request.body.data.date = moment().utc().toDate();
    request.body.data.date_surgery = moment(
      request.body.data.date_surgery,
      "DD/MM/YYYY"
    )
      .utc()
      .toDate();
...
router.get(
  "/surgery_sheet/recreate/pdf/:dateInit/:dateEnd",
