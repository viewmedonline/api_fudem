...
router.put("/persons", (request, response) => {
  ...
  if (request.body.surname) obj.surname = new RegExp(request.body.surname, "i");
  if (request.body.email) obj.email = request.body.email;
  if (request.body.gender) obj.gender = new RegExp(request.body.gender, "i");
  ...
  if (request.body._id) obj._id = request.body._id;
  if (request.body.id_document) obj.id_document = request.body.id_document;
  if (request.body.user) obj.user = { $in: request.body.user };
  if (request.body.type) obj.type = { $in: request.body.type };
  if (request.body.expfudemcare) obj.expfudemcare = request.body.expfudemcare;

  if (request.body.autoComplete) obj.idQflow = request.body.autoComplete;

  if (request.body.idQflow) obj.idQflow = request.body.idQflow;

  if (request.body.idQflow) obj.idQflow = request.body.idQflow;
  if (request.body.role) obj.role = { $in: request.body.role };
  if (request.body.address) {
    ...
    query = model.Person.find(obj)
      .or(objAddress)
      .sort({ "control.active": -1 })
      .populate("user");
  } else {
    query = model.Person.find(obj)
      .sort({ "control.active": -1 })
...
router.put("/persons/:personId", (request, response) => {
  model.Person.findByIdAndUpdate(request.params.personId, {
    $set: request.body,
  })
    .where("control.active")
    .equals(true)
...
router.post("/contacts/persons/list", (request, response) => {
  ...
  if (request.body.ids) {
    query = model.Person.find()
      .where("type")
      .equals(request.body.type)
      .where("user")
      .in(request.body.ids)
      .where("control.active")
      .equals(true);
...
router.put("/listPersons", (request, response) => {
  ...
  if (request.body.record) obj.record = request.body.record;
...
//
router.get("/persons/filter/:name", async (request, response) => {
  const { name } = request.params;
  try {
    query = await model.Person.find(
      {
    ...
    console.log("Microservice[person_filter]: " + error);
    response.status(500).json({
      status: "KO",
      message: "PersonSearchingProblem",
      documents: [],
