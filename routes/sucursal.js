...
router.post("/sucursal/list", (request, response) => {
  ...
  if (request.body.UnitId) obj.UnitId = request.body.UnitId;
  if (request.body.UnitType) obj.UnitType = request.body.UnitType;
  if (request.body.ParentUnitId) obj.ParentUnitId = request.body.ParentUnitId;
...
router.put("/sucursalUpdate/:sucursalId", (request, response) => {
  model.branchOffice
    .findByIdAndUpdate(request.params.sucursalId, { $set: request.body })
...
router.get("/sucursalListFudem", (request, response) => {
  let data = "";
  https
    .get("http://192.168.1.32/QFlow/Patients/Units.aspx", (resp) => {
      resp.on("data", (chunk) => {
        data += chunk;
      });
      resp.on("end", () => {
