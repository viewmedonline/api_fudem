      ...
      .catch((error) => {
        ...
        if (error.code === 11000) {
          response.status(400).json({
            status: "KO",
            message:
              "Ya existe una firma digital para esta persona con este archivo",
            documents: [],
          });
        } else {
...
router.get("/digital_signatures/person/:personId", (request, response) => {
  try {
    ...
    console.log(
      "Microservice[digital_signature_get_by_person_exception]: " + error
    );
    response.status(500).json({
      status: "KO",
      message: "Error interno del servidor",
...
router.get("/digital_signatures/:id", (request, response) => {
  try {
    ...
    console.log(
      "Microservice[digital_signature_get_by_id_exception]: " + error
    );
    response.status(500).json({
      status: "KO",
      message: "Error interno del servidor",
